...
public class EvalTestGenerator {
	...
	Map<Relationship, MyEdge> relsMap; //Map from the db relationships to my relationships in the gp
	...
	private GraphPattern translateToGP(){
		...
		for (Relationship r : rels){
			...
			relCount++;
			MyEdge relOpp = new MyEdge(target, source, type, relCount);
			relCount++;
			...
			if (!source.equals(target) && !gp.getAllEdges().contains(relOpp)){
				gp.addEdge(rel);
				relsMap.put(r, rel);
			}
		}
		...
		for (int i = 0; i < loopMax; i++){
			...
			if (!attr.equals("id") && !attr.equals("name") && !attr.equals("degree")){
				...
				if (nodeAttrs){
					...
					MyEdge edge = relsMap.get((Relationship)ent);
